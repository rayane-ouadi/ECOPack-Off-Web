document.addEventListener('DOMContentLoaded', () => {
    ...
    function getCurrentRoute() {
        const pageName = window.location.pathname.replace(/\\/g, '/').split('/').pop().toLowerCase();
    ...
    const animElements = document.querySelectorAll(
        '.slide-in-left, .slide-in-right, .fade-up, .animate-fade-up, .animate-slide-left, .animate-slide-right, .animate-scale-up'
    );
    ...
    if ('IntersectionObserver' in window) {
        ...
        animElements.forEach((element) => appearOnScroll.observe(element));
    } else {
        animElements.forEach((element) => element.classList.add('appear'));
    }
